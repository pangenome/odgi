...
// (note we can't put NodeSides in header, so leave this std::function static)
// Here and elsewhere we adopt the convention that a forward handle indicates
// the end of a node
typedef ska::flat_hash_set<handle_t> HandleSet;
typedef ska::flat_hash_map<handle_t, int> Handle2Component;
static void compute_side_components(const HandleGraph& graph,
                                    std::vector<HandleSet>& components,
    ...
    std::function<void(const handle_t& handle)> add_handle = [&](const handle_t& handle) {
 ...
 */
void addArbitraryTelomerePair(std::vector<stCactusEdgeEnd*> ends, stList *telomeres) {
...
std::pair<stCactusGraph*, stList*> handle_graph_to_cactus(PathHandleGraph& graph, const ska::flat_hash_set<std::string>& hint_paths) {
    ...
    std::vector<stCactusNode*> cactus_nodes(components.size());
    ...
    for (int i = 0; i < components.size(); ++i) {
        nid_t* cactus_node_id = (nid_t*)malloc(sizeof(nid_t));
        *cactus_node_id = i;
    ...
    // forward orientation.
    ska::flat_hash_map<nid_t, stCactusEdgeEnd*> edge_ends;
    for (int i = 0; i < components.size(); ++i) {
        ...
        for (auto side : components[i]) {
            ...
            nid_t node_id = graph.get_id(side);
            nid_t other_node_id = graph.get_id(other_side);
...
#ifdef debug
                cerr << "Creating cactus edge for sides " << node_id << "/" << is_end << " -- " << other_node_id << "/" << other_is_end << ": " << i << " -> " << j << endl;
#endif
    ...
    std::vector<ska::flat_hash_set<nid_t>> weak_components = algorithms::weakly_connected_components(&graph);
       
    // We also want a map so we can efficiently find which component a node lives in.
    ska::flat_hash_map<nid_t, size_t> node_to_component;
    ...
    std::vector<ska::flat_hash_set<handle_t>> component_tips(weak_components.size());
    ...
    std::vector<std::vector<std::string>> component_paths(weak_components.size());
    // Also get the path length.
    ska::flat_hash_map<std::string, size_t> path_length;
    ...
    std::vector<std::vector<ska::flat_hash_set<nid_t>>> component_strong_components(weak_components.size());
    ...
    // This std::function adds as telomeres a pair of inward-facing handles.
    ...
    for (size_t i = 0; i < weak_components.size(); i++) {
        ...
        // two tips at its ends.
        {
            ...
            std::pair<handle_t, handle_t> longest_path_tips;
                ...
                for (auto& path_name : component_paths[i]) {
                    ...
                    if (component_tips[i].count(path_start) && component_tips[i].count(path_end)) {
                        ...
                        if (path_length[path_name] > longest_path_length) {
                            ...
                            longest_path_tips = std::make_pair(path_start, path_end);
        ...
        if (component_tips[i].size() <= 10) {
            ...
            // Pairs are stored lowest-node-and-orientation first. Really we want a hashable ska::flat_hash_pair...
            ska::flat_hash_map<std::pair<handle_t, handle_t>, size_t> tip_distances;
            
            // Define a std::function to look up in the memo, or do Dijkstra if necessary
            // Takes a pair that must be sorted already.
            auto get_or_compute_distance = [&](std::pair<handle_t, handle_t>& key) {
                if (!tip_distances.count(key)) {
                    ...
                    ska::flat_hash_map<handle_t, size_t> distances = algorithms::find_shortest_paths(&graph, key.first);
                    ...
                    for (auto& other_tip : component_tips[i]) {
                        ...
                        std::pair<handle_t, handle_t> other_key = std::make_pair(key.first, other_tip);
            ...
            std::pair<handle_t, handle_t> furthest;
            // And their distance
            ...
            for (auto& tip1 : component_tips[i]) {
                for (auto& tip2 : component_tips[i]) {
                    ...
                    std::pair<handle_t, handle_t> key = std::make_pair(tip1, tip2);
            ...
            if (furthest_distance == numeric_limits<size_t>::max()) {
                ...
                for (auto& tip : component_tips[i]) {
                    ...
                    std::pair<handle_t, handle_t> key = std::make_pair(tip, tip);
            ...
            std::vector<nid_t> component_nodes(weak_components[i].begin(), weak_components[i].end());
            ...
            ska::flat_hash_map<handle_t, size_t> distances_right = algorithms::find_shortest_paths(&graph, start);
            ska::flat_hash_map<handle_t, size_t> distances_left = algorithms::find_shortest_paths(&graph, graph.flip(start));
            ...
            if (furthest_right_distance != numeric_limits<size_t>::max() && furthest_left_distance != numeric_limits<size_t>::max()) {
                ...
                std::vector<handle_t> best_tips{furthest_left_tip, furthest_right_tip};
                ...
                while(true) {
                    ...
                    ska::flat_hash_map<handle_t, size_t> distances = algorithms::find_shortest_paths(&graph, best_tips[starting_tip]);
        ...
        // want to try it both ways and compare.
        {
            if (component_tips[i].size() >= 2) {
                ...
                std::vector<handle_t> tips{component_tips[i].begin(), component_tips[i].end()};
        ...
        // node.
        {
            ...
            std::vector<nid_t> sorted_component{strong_components[largest_component].begin(),
                strong_components[largest_component].end()};
            std::sort(sorted_component.begin(), sorted_component.end());
            nid_t break_node = sorted_component.front();
            ...
            std::vector<handle_t> dfs_sources{break_handle};
            // Don't continue backward through this node.
            ska::flat_hash_set<handle_t> dfs_sinks{graph.flip(break_handle)};
            
            // We set this to true if we find an edge to the forward version of break_handle.
            bool found = false;
            
            std::function<void(const handle_t&)> handle_noop = [](const handle_t&) {
            };
            
            std::function<void(const edge_t&)> edge_noop = [](const edge_t&) {
            };
            
            std::function<bool(void)> dfs_break = [&]() {
    ...
    return std::make_pair(cactus_graph, telomeres);
}
