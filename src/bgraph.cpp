...
namespace betagraph{
    ...
    handle_t BGraph::get_handle(const dankgraph::id_t& node_id, bool is_reverse) const{
        return hhelper.pack(node_id, is_reverse);
    }
    

    dankgraph::id_t BGraph::get_id(const handle_t& handle) const{
    ...
    dankgraph::id_t BGraph::min_node_id(void) const{
        return graph.min_node_id;
    }

    dankgraph::id_t BGraph::max_node_id(void) const{
...
//     handle_t create_handle(const std::string& sequence, const dankgraph::id_t& id);
...
//     inline void create_edge(const dankgraph::edge_t& edge) {
