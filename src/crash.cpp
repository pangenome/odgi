...
void stacktrace_manually(std::ostream& out, int signalNumber, void* ip, void** bp) {
        ...
        if (address_library.dli_sname != nullptr) {
            ...
            if(status == 0) {
                ...
                out << "Address " << ip << " in demangled symbol " << demangledName
                    ...
                    << " at offset " << (void*)((size_t)ip - ((size_t)address_library.dli_fbase)) << std::endl;
                free(demangledName);
            } else {
                // Leave mangled
                out << "Address " << ip << " in mangled symbol " << address_library.dli_sname
                    ...
                    out << "Running " << command.str() << "..." << std::endl;
                    system(command.str().c_str());
                #endif
            #endif
            
        } else {
            out << "Address " << ip << " out of symbol in library " << address_library.dli_fname << std::endl;
        }

        if(address_library.dli_sname != nullptr && !strcmp(address_library.dli_sname, "main")) {
            out << "Stack hit main" << std::endl;
            break;
        }
    ...
    out << "Stack trace complete" << std::endl;
    out << std::endl;
...
void emit_stacktrace(int signalNumber, siginfo_t *signalInfo, void *signalContext) {
    ...
    *out << "Crash report for dg " << std::endl; //<< Version::get_short() << std::endl;
    ...
    void** bp;
        ...
        *out << "Caught signal " << signalNumber << " raised at address " << ip << std::endl;
    ...
    if (fullTrace == false) {
        std::cerr << "ERROR: Signal "<< signalNumber << " occurred. dg has crashed." << std::endl;
        // Print path for stack trace file
        std::cerr << "Stack trace path: "<< dirName << "/stacktrace.txt" << std::endl;
    }
